...
import Snowfall from "../../files (10)/Snowfall_COMPONENT";
...
const ProductForm = ({ isEdit = false }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    category: "",
    stockQuantity: "",
    unit: "kilogram", // ✅ FIX 1: Set default value
  ...
  const [existingImageUrl, setExistingImageUrl] = useState(null); // ✅ FIX 2: Track existing image
      ...
      const fetchProduct = async () => {
        try {
          ...
          setFormData({
            ...
            unit: product.unit || "kilogram", // ✅ FIX 1: Load unit from backend
            productAvailable: product.productAvailable ?? true,
            sections: product.sections || "all-products",
          });

          // ✅ FIX 2: Load existing image URL
          if (product.imageUrl) {
          ...
          // Parse sections
          if (product.sections) {
            const sectionsArray = product.sections.split(",").map((s) => s.trim());
            setSelectedSections(sectionsArray.length > 0 ? sectionsArray : ["all-products"]);
          }
        } catch (error) {
          console.error("Error fetching product:", error);
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ FIX 2: Only append image if a new one is selected
    if (image) {
    ...
    // ✅ FIX 1: Ensure unit is included in the product data
    const productData = {
      ...formData,
      unit: formData.unit || "kilogram", // Force default if somehow empty
    ...
    <div className="product-form-page">
      ...
      <form onSubmit={handleSubmit} className="product-form">
        ...
        <div className="form-row">
          ...
          {/* ✅ FIX 1: Properly controlled unit select */}
          <div className="form-group">
            <label>Unit</label>
            <select
              value={formData.unit}
        ...
        {/* ✅ FIX 2: Image preview with existing image support */}
        <div className="form-group">
          ...
          <input
            ...
            accept="image/*"
            // ✅ FIX 2: Make it optional when editing
